...
export function parseModelResponse(
  ...
  if (modelResp.phase === "clarification_needed") {
    ...
    try {
      ...
      let visionSuggestion: DailyResetVisionSuggestion | undefined = undefined;
      try {
        const rawVision = draft.visionSuggestion || (rawResponse as any)?.visionSuggestion;
        if (rawVision && typeof rawVision === "object") {
          const confidence = rawVision.confidence;
          if (confidence === "medium" || confidence === "high") {
            const suggestedTitle = typeof rawVision.suggestedTitle === "string" ? rawVision.suggestedTitle.trim() : "";
            const desiredOutcome = typeof rawVision.desiredOutcome === "string" ? rawVision.desiredOutcome.trim() : "";
            const reason = typeof rawVision.reason === "string" ? rawVision.reason.trim() : "";

            let mappedSourceIds: string[] = [];
            if (Array.isArray(rawVision.sourceItemIndexes)) {
              for (const idx of rawVision.sourceItemIndexes) {
                if (typeof idx === "number" && Number.isInteger(idx) && idx >= 0 && idx < classifiedItems.length) {
                  mappedSourceIds.push(canonicalClassifiedIds[idx]);
                }
              }
            } else if (Array.isArray(rawVision.sourceItemIds)) {
              for (const id of rawVision.sourceItemIds) {
                if (canonicalClassifiedIds.includes(String(id))) {
                  mappedSourceIds.push(String(id));
                }
              }
            }

            if (
              suggestedTitle.length >= 3 && suggestedTitle.length <= 200 &&
              desiredOutcome.length >= 3 && desiredOutcome.length <= 500 &&
              reason.length >= 3 && reason.length <= 500 &&
              mappedSourceIds.length > 0
            ) {
              const needsClarification = Boolean(rawVision.needsClarification);
              let clarificationQuestion: string | undefined = undefined;
              if (rawVision.clarificationQuestion && typeof rawVision.clarificationQuestion === "string") {
                const cq = rawVision.clarificationQuestion.trim();
                if (cq.length > 0 && cq.length <= 300) {
                  clarificationQuestion = cq;
                }
              }
              let possibleExistingVisionId: string | undefined = undefined;
              if (rawVision.possibleExistingVisionId && typeof rawVision.possibleExistingVisionId === "string") {
                const pevId = rawVision.possibleExistingVisionId.trim();
                if (pevId.length > 0 && pevId.length <= 100) {
                  possibleExistingVisionId = pevId;
                }
              }
        ...
        // Reject invalid vision suggestion quietly without breaking plan
      ...
      return {
        ...
        phase: "plan_ready",
        draft: planDraft,
        visionSuggestion: planDraft.visionSuggestion
      };
    } catch (err: any) {

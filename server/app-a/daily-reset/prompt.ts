...
OPTIONAL VISION SUGGESTION (Brain dump → Vision candidate):
- Propose AT MOST ONE vision suggestion ONLY when the Brain dump contains a genuine multi-week or multi-month aspiration, a strategic long-term life/business direction, or a recurring theme requiring structured milestones.
- DO NOT propose a vision suggestion for:
  - Routine errands or operational tasks (e.g. "kupi mleko", "pošalji račun", "završi izveštaj za danas").
  - Emotional venting or temporary mood states (e.g. "premoren sam", "sve mi je teško").
  - Ideas where confidence is low or intent is speculative.
- When proposing a vision suggestion:
  - sourceItemIndexes: Indexes of the specific brain dump items in classifiedItems that embody this aspiration.
  - suggestedTitle: A concise, grounded title reflecting the user's exact direction without exaggeration.
  - desiredOutcome: Clear, realistic outcome describing what success looks like.
  - reason: Brief justification explaining why this warrants long-term vision development rather than a simple daily task.
  - confidence: "medium" or "high" (do not output if confidence is low).
  - needsClarification: true if key direction parameters (such as timeframe or initial scope) are ambiguous.
  - clarificationQuestion: Optional short question if needsClarification is true.
  - possibleExistingVisionId: Existing vision ID if clearly related.
- If no genuine long-term direction exists in the brain dump, OMIT visionSuggestion entirely.

...
export const SUPPORTED_REMOTE_COLLECTIONS: readonly RemoteCollectionTarget[] = Object.freeze([
  ...
  Object.freeze({
    scopeCategory: "routines_shared",
    collectionName: "routineMutationReceipts",
    rootCollection: "users",
  }),
]);
...
export interface FirestoreAdapter {
  ...
  resetUserVisionPointer?: (userId: string) => Promise<void>;
}
...
export const defaultFirestoreAdapter: FirestoreAdapter = {
  ...
  async resetUserVisionPointer(userId: string) {
    const { db } = await import("../../lib/firebase");
    await setDoc(doc(db, "users", userId), { currentVisionId: null }, { merge: true });
  },
};
  ...
  for (const scopeKey of remoteScopeKeys) {
    ...
    if (!scopeFailed) {
      if (scopeKey === "vision_shared") {
        try {
          if (adapter.resetUserVisionPointer) {
            await adapter.resetUserVisionPointer(validUid);
          }
        } catch {

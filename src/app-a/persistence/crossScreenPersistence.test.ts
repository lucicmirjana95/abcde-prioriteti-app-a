...
// 1. Planning routine A
start();
const planned1 = await repo.addPlannedRoutineForDate('user-1', document.localDate, 'r_routine_a');
assert.deepEqual(planned1, ['r_routine_a']);
assert.deepEqual(repo.read(path).plan.plannedRoutineIds, ['r_routine_a']); scenarios++;

// 2 & 3. Reload through production load confirms A remains planned
const loaded1 = await repo.loadPlannedRoutineIds('user-1', document.localDate);
assert.deepEqual(loaded1, ['r_routine_a']); scenarios++;

// 4. Adding B does not remove A
const planned2 = await repo.addPlannedRoutineForDate('user-1', document.localDate, 'r_routine_b');
assert.deepEqual(planned2, ['r_routine_a', 'r_routine_b']);
assert.deepEqual(repo.read(path).plan.plannedRoutineIds, ['r_routine_a', 'r_routine_b']); scenarios++;

// 5. Re-adding A does not create duplicate
const planned3 = await repo.addPlannedRoutineForDate('user-1', document.localDate, 'r_routine_a');
assert.deepEqual(planned3, ['r_routine_a', 'r_routine_b']); scenarios++;

// 6. Removing A leaves B
const planned4 = await repo.removePlannedRoutineForDate('user-1', document.localDate, 'r_routine_a');
assert.deepEqual(planned4, ['r_routine_b']);
assert.deepEqual(repo.read(path).plan.plannedRoutineIds, ['r_routine_b']); scenarios++;

// 7. Reload confirms A is no longer planned
const loaded2 = await repo.loadPlannedRoutineIds('user-1', document.localDate);
assert.deepEqual(loaded2, ['r_routine_b']); scenarios++;

// 8. Removing routine doesn't alter completion, status, frequency or other plan items
assert.equal(repo.read(path).plan.firstFocus[0].title, 'Write introduction');
assert.equal(repo.read(path).localDate, '2026-09-07'); scenarios++;

// 9. Stale revision / concurrency check
repo.seed(path, { ...document, revision: 10, plan: { ...plan, plannedRoutineIds: ['r_routine_b'] } });
await repo.addPlannedRoutineForDate('user-1', document.localDate, 'r_routine_c');
assert.equal(repo.read(path).revision, 11);
assert.deepEqual(repo.read(path).plan.plannedRoutineIds, ['r_routine_b', 'r_routine_c']); scenarios++;

// 10. Toggle helper behaves idempotently
const toggledOff = await repo.togglePlannedRoutineForDate('user-1', document.localDate, 'r_routine_b', false);
assert.deepEqual(toggledOff, ['r_routine_c']);
const toggledOn = await repo.togglePlannedRoutineForDate('user-1', document.localDate, 'r_routine_d', true);
assert.deepEqual(toggledOn, ['r_routine_c', 'r_routine_d']); scenarios++;

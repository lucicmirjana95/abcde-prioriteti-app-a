...
const CLARIFY_COPY = {
  en: { clarify: "Clarify", prompt: "What concrete action do you want to take?", save: "Save as task", help: "Help me clarify", keep: "Keep as note" },
  sr: { clarify: "Razjasni", prompt: "Koju konkretnu radnju želite da preduzmete?", save: "Sačuvaj kao zadatak", help: "Pomozi mi da razjasnim", keep: "Zadrži kao belešku" },
  tr: { clarify: "Netleştir", prompt: "Hangi somut eylemi yapmak istiyorsunuz?", save: "Görev olarak kaydet", help: "Netleştirmeme yardım et", keep: "Not olarak tut" },
} as const;
  ...
  return <div className="mx-auto w-full max-w-[760px] px-4 pb-10 sm:px-6">
    ...
    {visible.length === 0 ? <div className="app-a-surface flex min-h-[180px] flex-col items-center justify-center gap-3 p-7 text-center"><Inbox className="h-6 w-6" style={{ color: "var(--app-a-accent)" }} /><p style={{ color: "var(--app-a-text-secondary)" }}>{t.empty}</p></div> : <div className="space-y-2.5">{visible.map((item) => {
      ...
      return <article key={item.id} className="app-a-surface min-w-0 p-4">
        ...
        {clarifyingId === item.id ? <div className="mt-3 rounded-xl border p-3" style={{ borderColor: "var(--app-a-border)" }}><label className="block text-[13px] font-semibold">{CLARIFY_COPY[language].prompt}<input autoFocus value={clarifiedAction} maxLength={500} onChange={event => setClarifiedAction(event.target.value)} className="app-a-field app-a-focus-ring mt-2 w-full px-3 py-2.5 text-[16px]" /></label>{clarificationHelp ? <div className="mt-3 rounded-lg p-3 text-[13px]" style={{ background: "var(--app-a-surface-secondary)" }}>{clarificationHelp.questions.length ? <ul className="mb-3 list-disc space-y-1 pl-5">{clarificationHelp.questions.map(question => <li key={question}>{question}</li>)}</ul> : null}<div className="flex flex-wrap gap-2">{clarificationHelp.suggestions.map(suggestion => <button key={suggestion} type="button" onClick={() => setClarifiedAction(suggestion)} className="app-a-secondary-button min-h-10 px-3 text-left text-[12px]">{suggestion}</button>)}</div></div> : null}<div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={clarifiedAction.trim().length < 3 || Boolean(processing)} onClick={() => void run(item.id, async () => { const next = await convertInboxNoteToTask(user.uid, item.id, clarifiedAction); setItems(all => all.map(entry => entry.id === item.id ? next : entry)); setClarifyingId(null); setClarifiedAction(""); setClarificationHelp(null); setSearch(""); setFilter("all"); setNotice(t.converted); })} className="app-a-primary-button px-3 text-[13px]">{CLARIFY_COPY[language].save}</button><button type="button" disabled={clarificationLoading} onClick={() => { setClarificationLoading(true); void clarifyInboxNote(item.title, language).then(setClarificationHelp).catch(() => setError(t.error)).finally(() => setClarificationLoading(false)); }} className="app-a-secondary-button px-3 text-[13px]">{clarificationLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : null}{CLARIFY_COPY[language].help}</button><button type="button" onClick={() => { setClarifyingId(null); setClarificationHelp(null); }} className="app-a-secondary-button px-3 text-[13px]">{clarifiedAction ? t.cancel : CLARIFY_COPY[language].keep}</button></div></div> : null}

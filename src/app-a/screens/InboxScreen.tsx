...
import { Archive, CalendarPlus, Check, Clock3, FileText, Inbox, Loader2, Plus, Search, Trash2, Undo2 } from "lucide-react";
...
const COPY = {
  en: { eyebrow: "Not for today yet", title: "Inbox", intro: "Keep later tasks clear and decide what happens next.", add: "Add item", placeholder: "What do you want to remember?", search: "Search Inbox", all: "Tasks", notes: "To clarify", note: "Note — no action assumed", makeTask: "Turn into task", week: "This week", later: "Later", waiting: "Waiting", scheduled: "Scheduled", completed: "Completed", archived: "Archived", empty: "Nothing matches this view.", minutes: "min", duration: "Minutes", addToday: "Add to Today", schedule: "Schedule", wait: "Waiting", restore: "Move to Inbox", complete: "Mark complete outside today's plan", archive: "Archive", delete: "Delete", confirmDelete: "Delete permanently?", cancel: "Cancel", noPlan: "Create today's plan before adding this item.", durationNeeded: "Add an estimated duration first.", capacity: "This item does not fit in today's available time.", duplicate: "This item is already in today's plan.", error: "The action could not be completed. Try again.", signInError: "Sign-in was not completed. Your data has not changed.", retrySignIn: "Try sign-in again" },
  sr: { eyebrow: "Još nije za danas", title: "Inboks", intro: "Sačuvajte obaveze za kasnije i razjasnite beleške bez izmišljanja zadataka.", add: "Dodaj stavku", placeholder: "Šta želite da zapamtite?", search: "Pretraži Inboks", all: "Zadaci", notes: "Za razjašnjenje", note: "Beleška — nije pretpostavljena radnja", makeTask: "Pretvori u zadatak", week: "Ove nedelje", later: "Kasnije", waiting: "Čekam", scheduled: "Zakazano", completed: "Završeno", archived: "Arhivirano", empty: "Nema stavki u ovom prikazu.", minutes: "min", duration: "Minuta", addToday: "Dodaj u Danas", schedule: "Zakaži", wait: "Čekam", restore: "Vrati u Inboks", complete: "Označi završeno van današnjeg plana", archive: "Arhiviraj", delete: "Obriši", confirmDelete: "Trajno obrisati?", cancel: "Otkaži", noPlan: "Prvo napravite današnji plan.", durationNeeded: "Prvo dodajte procenjeno trajanje.", capacity: "Ova stavka ne staje u raspoloživo vreme za danas.", duplicate: "Ova stavka je već u današnjem planu.", error: "Radnja nije uspela. Pokušajte ponovo.", signInError: "Prijava nije završena. Vaši podaci nisu promenjeni.", retrySignIn: "Pokušaj prijavu ponovo" },
  tr: { eyebrow: "Henüz bugün için değil", title: "Gelen kutusu", intro: "Sonraki görevleri saklayın ve görev uydurmadan notları netleştirin.", add: "Öğe ekle", placeholder: "Neyi hatırlamak istiyorsunuz?", search: "Gelen kutusunda ara", all: "Görevler", notes: "Netleştirilecek", note: "Not — eylem varsayılmadı", makeTask: "Göreve dönüştür", week: "Bu hafta", later: "Daha sonra", waiting: "Bekliyor", scheduled: "Planlandı", completed: "Tamamlandı", archived: "Arşivlenmiş", empty: "Bu görünümde öğe yok.", minutes: "dk", duration: "Dakika", addToday: "Bugüne ekle", schedule: "Planla", wait: "Bekliyor", restore: "Gelen kutusuna taşı", complete: "Bugünkü plan dışında tamamlandı olarak işaretle", archive: "Arşivle", delete: "Sil", confirmDelete: "Kalıcı olarak silinsin mi?", cancel: "İptal", noPlan: "Bu öğeyi eklemeden önce bugünün planını oluşturun.", durationNeeded: "Önce tahmini süre ekleyin.", capacity: "Bu öğe bugünkü kullanılabilir süreye sığmıyor.", duplicate: "Bu öğe zaten bugünün planında.", error: "İşlem tamamlanamadı. Tekrar deneyin.", signInError: "Giriş tamamlanmadı. Verileriniz değişmedi.", retrySignIn: "Girişi tekrar dene" },
} as const;
...
export default function InboxScreen({ language, preferences }: { language: AppALanguage; preferences: AppAPreferences }) {
  ...
  const filters: Array<[Filter, string]> = [["all", t.all], ["notes", t.notes], ["this_week", t.week], ["later", t.later], ["waiting", t.waiting], ["scheduled", t.scheduled], ["completed", t.completed], ["archived", t.archived]];
  ...
  return <div className="mx-auto w-full max-w-[760px] px-5 pb-8 sm:px-6">
    ...
    {visible.length === 0 ? <div className="app-a-surface flex min-h-[200px] flex-col items-center justify-center gap-3 p-8 text-center"><Inbox className="h-6 w-6" style={{ color: "var(--app-a-accent)" }} /><p style={{ color: "var(--app-a-text-secondary)" }}>{t.empty}</p></div> : <div className="space-y-3">{visible.map((item) => <article key={item.id} className="app-a-surface p-4 sm:p-5">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words text-[16px] font-semibold">{item.title}</h2><div className="mt-1 flex flex-wrap gap-2 text-[12px]" style={{ color: "var(--app-a-text-secondary)" }}>{item.kind === "note" ? <span className="inline-flex items-center gap-1"><FileText className="h-3.5 w-3.5" />{t.note}</span> : <>{item.estimatedMinutes ? <span className="inline-flex items-center gap-1"><Clock3 className="h-3.5 w-3.5" />{item.estimatedMinutes} {t.minutes}</span> : null}<span>{item.status === "waiting" ? t.waiting : item.status === "scheduled" ? `${t.scheduled}: ${item.scheduledLocalDate}` : item.status === "completed" ? t.completed : item.status === "archived" ? t.archived : item.horizon === "this_week" ? t.week : t.later}</span></>}</div></div>{processing === item.id ? <Loader2 className="h-4 w-4 animate-spin" /> : null}</div>
      {!item.estimatedMinutes && item.kind !== "note" && item.status !== 'completed' && item.status !== 'archived' ? <div className="mt-3 flex flex-wrap items-end gap-2"><label className="text-[14px]">{t.duration}<input type="number" min="1" max="1440" className="app-a-field ml-2 min-h-11 w-24 px-3 text-[16px]" value={missingMinutes[item.id] || ''} onChange={event => setMissingMinutes(values => ({ ...values, [item.id]: event.target.value }))} /></label><button type="button" disabled={!!processing || !missingMinutes[item.id]} className="app-a-secondary-button app-a-focus-ring px-4" onClick={() => void run(item.id, async () => { const next = await addMissingInboxDuration(user.uid, item.id, Number(missingMinutes[item.id])); setItems(values => values.map(entry => entry.id === item.id ? next : entry)); })}>{language === 'sr' ? 'Sačuvaj trajanje' : language === 'tr' ? 'Süreyi kaydet' : 'Save duration'}</button></div> : null}
      <div className="mt-4 flex flex-wrap gap-2 border-t pt-3" style={{ borderColor: "var(--app-a-border)" }}>{(item.status === "inbox" || item.status === "waiting") && item.estimatedMinutes ? <button type="button" onClick={() => void addToday(item)} disabled={!!processing} className="app-a-primary-button app-a-focus-ring px-3 text-[12px]"><CalendarPlus className="h-4 w-4" />{t.addToday}</button> : null}
        {item.kind === "note" ? <button type="button" onClick={() => void run(item.id, async () => { const next = await convertInboxNoteToTask(user.uid, item.id); setItems((all) => all.map((entry) => entry.id === item.id ? next : entry)); })} className="app-a-primary-button app-a-focus-ring px-3 text-[12px]">{t.makeTask}</button> : item.status === "inbox" ? <><div className="app-a-secondary-button flex min-h-11 items-center gap-1 px-2 text-[12px]"><input type="date" min={getLocalDateKeyInTimeZone(effectiveTimeZone)} value={scheduleFor[item.id] || ""} onChange={(event) => setScheduleFor((all) => ({ ...all, [item.id]: event.target.value }))} className="max-w-[125px] bg-transparent" aria-label={t.schedule} /><button type="button" disabled={!scheduleFor[item.id]} onClick={() => void update(item, "scheduled", { scheduledLocalDate: scheduleFor[item.id] })}>{t.schedule}</button></div><button type="button" onClick={() => void update(item, "waiting")} className="app-a-secondary-button app-a-focus-ring px-3 text-[12px]">{t.wait}</button></> : <button type="button" onClick={() => void update(item, "inbox")} className="app-a-secondary-button app-a-focus-ring px-3 text-[12px]"><Undo2 className="h-4 w-4" />{t.restore}</button>}
        {item.kind !== "note" && item.status !== "completed" && item.status !== "archived" ? <><button type="button" onClick={() => void update(item, "completed")} className="app-a-secondary-button app-a-focus-ring px-3 text-[12px]"><Check className="h-4 w-4" />{t.complete}</button><button type="button" onClick={() => void update(item, "archived")} className="app-a-secondary-button app-a-focus-ring px-3 text-[12px]"><Archive className="h-4 w-4" />{t.archive}</button></> : null}

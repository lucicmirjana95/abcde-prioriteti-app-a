...
import { deleteInboxItem, importDailyPlanItemsToInbox, loadInboxItems, saveInboxItem, savePlanAndScheduleInboxItemAtomic, updateInboxItemStatus } from "../persistence/inboxRepository";
...
type Filter = "all" | "this_week" | "later" | "waiting" | "scheduled" | "completed" | "archived";
const COPY = {
...
export default function InboxScreen({ language, preferences }: { language: AppALanguage; preferences: AppAPreferences }) {
  ...
  const visible = useMemo(() => items.filter((item) => {
    ...
    const matches = filter === "all" ? !isTerminal : (filter === "this_week" && item.horizon === "this_week" && item.status === "inbox") || (filter === "later" && item.horizon === "later" && item.status === "inbox") || item.status === filter;
  ...
  const filters: Array<[Filter, string]> = [["all", t.all], ["this_week", t.week], ["later", t.later], ["waiting", t.waiting], ["scheduled", t.scheduled], ["completed", t.completed], ["archived", t.archived]];
  ...
  return <div className="mx-auto w-full max-w-[760px] px-5 pb-8 sm:px-6">
    ...
    {visible.length === 0 ? <div className="app-a-surface flex min-h-[200px] flex-col items-center justify-center gap-3 p-8 text-center"><Inbox className="h-6 w-6" style={{ color: "var(--app-a-accent)" }} /><p style={{ color: "var(--app-a-text-secondary)" }}>{t.empty}</p></div> : <div className="space-y-3">{visible.map((item) => <article key={item.id} className="app-a-surface p-4 sm:p-5">
      ...
      <div className="mt-4 flex flex-wrap gap-2 border-t pt-3" style={{ borderColor: "var(--app-a-border)" }}>{(item.status === "inbox" || item.status === "waiting") && item.estimatedMinutes ? <button type="button" onClick={() => void addToday(item)} disabled={!!processing} className="app-a-primary-button app-a-focus-ring px-3 text-[12px]"><CalendarPlus className="h-4 w-4" />{t.addToday}</button> : null}
        ...
        {item.status !== "completed" && item.status !== "archived" ? <><button type="button" onClick={() => void update(item, "completed")} className="app-a-secondary-button app-a-focus-ring px-3 text-[12px]"><Check className="h-4 w-4" />{t.complete}</button><button type="button" onClick={() => void update(item, "archived")} className="app-a-secondary-button app-a-focus-ring px-3 text-[12px]"><Archive className="h-4 w-4" />{t.archive}</button></> : null}

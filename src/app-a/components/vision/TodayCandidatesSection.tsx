...
const COPY = {
  en: { title: "Actions from Vision", intro: "Suggested next steps from your active goals. You decide what to add today.", add: "Add to today’s plan", createPlan: "Create today’s plan", reviewPlan: "Review and save today’s plan", noPlan: "Create today’s plan to add this step.", draftPlan: "Review and save today’s plan before adding another step.", dismiss: "Remove step", retry: "Try again", error: "Vision steps could not be loaded.", duplicate: "This step is already in the plan.", capacity_unknown: "Set your available time before adding this step.", capacity_exceeded: "Today’s plan is full. Keep this step here for later or adjust the plan.", invalid_plan: "This step could not be added. Try again." },
  sr: { title: "Akcije iz Vizije", intro: "Predloženi sledeći koraci iz aktivnih ciljeva. Vi birate šta ćete dodati danas.", add: "Dodaj u današnji plan", createPlan: "Napravi današnji plan", reviewPlan: "Pregledaj i sačuvaj današnji plan", noPlan: "Napravite današnji plan da biste dodali ovaj korak.", draftPlan: "Pregledajte i sačuvajte današnji plan pre dodavanja novog koraka.", dismiss: "Ukloni korak", retry: "Pokušaj ponovo", error: "Koraci iz Vizije nisu učitani.", duplicate: "Ovaj korak je već u planu.", capacity_unknown: "Unesite raspoloživo vreme pre dodavanja ovog koraka.", capacity_exceeded: "Današnji plan je popunjen. Sačuvajte ovaj korak za kasnije ili prilagodite plan.", invalid_plan: "Ovaj korak nije dodat. Pokušajte ponovo." },
  tr: { title: "Vizyondan eylemler", intro: "Aktif hedeflerinizden önerilen sonraki adımlar. Bugün ne ekleyeceğinize siz karar verirsiniz.", add: "Bugünün planına ekle", createPlan: "Bugünün planını oluştur", reviewPlan: "Bugünün planını gözden geçir ve kaydet", noPlan: "Bu adımı eklemek için bugünün planını oluşturun.", draftPlan: "Yeni bir adım eklemeden önce bugünün planını gözden geçirip kaydedin.", dismiss: "Adımı kaldır", retry: "Tekrar dene", error: "Vizyon adımları yüklenemedi.", duplicate: "Bu adım zaten planda.", capacity_unknown: "Bu adımı eklemeden önce kullanılabilir sürenizi belirleyin.", capacity_exceeded: "Bugünün planı dolu. Bu adımı daha sonrası için burada tutun veya planı düzenleyin.", invalid_plan: "Bu adım eklenemedi. Tekrar deneyin." },
} as const;
...
export default function TodayCandidatesSection({ userId, language, planState, onPlanAction, onAddToPlan }: Props) {
  ...
  const durationLabel = language === "sr" ? "Koliko minuta želiš za ovaj korak?" : language === "tr" ? "Bu adım için kaç dakika ayırmak istersin?" : "How many minutes for this step?";
  ...
  async function add(item: TodayCandidate) {
    setBusyId(item.id); setError(null);
    try {
      const minutes = Number(durations[item.id] ?? (item.estimatedMinutes || ""));
      if (!Number.isInteger(minutes) || minutes < 1 || minutes > 480) { setError("duration_required"); return; }
  ...
  const errorText = error === "duration_required" ? durationLabel : error ? (t[error as keyof typeof t] || t.error) : null;
  ...
  return <section className="mx-auto mt-7 w-full max-w-[760px] px-5 pb-2 sm:px-6" aria-labelledby="today-vision-heading">
    ...
    {errorText ? <div role="alert" className="app-a-panel-danger mt-3 flex flex-wrap items-center justify-between gap-2 text-[13px]"><span>{errorText}</span>{error === "error" ? <button type="button" onClick={() => setRefreshVersion((value) => value + 1)} className="app-a-focus-ring min-h-11 rounded-lg px-3 font-semibold">{t.retry}</button> : null}</div> : null}
    ...
    <div className="mt-3 space-y-2">{items.map((item) => <article key={item.id} className="app-a-surface flex items-start gap-3 p-4"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#AF52DE]/10 text-[#AF52DE]"><Compass className="h-5 w-5" aria-hidden="true" /></span><div className="min-w-0 flex-1"><h3 className="text-[15px] font-semibold">{item.title}</h3><p className="mt-1 flex items-center gap-1 text-[13px]" style={{ color: "var(--app-a-text-secondary)" }}><Clock3 className="h-3.5 w-3.5" aria-hidden="true" /><label className="flex flex-wrap items-center gap-2">{durationLabel}<input type="number" min="1" max="480" value={durations[item.id] ?? (item.estimatedMinutes || "")} onChange={(event) => setDurations((values) => ({ ...values, [item.id]: event.target.value }))} className="app-a-field min-h-11 w-24 px-3 text-[16px]" /></label></p>{planState === "confirmed" ? <button type="button" disabled={busyId !== null} onClick={() => void add(item)} className="app-a-primary-button app-a-focus-ring mt-3 gap-2 px-3 text-[13px]"><CalendarPlus className="h-4 w-4" aria-hidden="true" />{t.add}</button> : null}</div><button type="button" disabled={busyId !== null} onClick={() => void dismiss(item)} className="app-a-focus-ring flex h-11 w-11 shrink-0 items-center justify-center rounded-full" aria-label={t.dismiss}><X className="h-4 w-4" aria-hidden="true" /></button></article>)}</div>
  </section>;
}

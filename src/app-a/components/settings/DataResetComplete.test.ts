...
// STRICT TEST ISOLATION: Tests must immediately fail if any unexpected network attempt is made
function failOnNetwork(target: string): never {
  assert.fail(`STRICT ISOLATION FAILURE: Test attempted network access to ${target}`);
}

http.request = (() => failOnNetwork("http.request")) as any;
http.get = (() => failOnNetwork("http.get")) as any;
https.request = (() => failOnNetwork("https.request")) as any;
https.get = (() => failOnNetwork("https.get")) as any;
net.connect = (() => failOnNetwork("net.connect")) as any;
if (typeof globalThis.fetch === "function") {
  globalThis.fetch = (async (url: any) => {
    failOnNetwork(`fetch(${String(url)})`);
  }) as any;
}
...
async function runCompleteDataResetTest() {
  ...
  const createMockAdapter = (failPath?: string): FirestoreAdapter => {
    return {
      ...
      resetUserVisionPointer: async (_uid: string) => {
        pointerResetCount++;
      },
    };
  };
  ...
  assert.strictEqual(pointerResetCount, 0, "Vision pointer must not be reset on partial failure");
  ...
  assert.strictEqual(pointerResetCount, 1, "Vision pointer is reset exactly once after successful retry");

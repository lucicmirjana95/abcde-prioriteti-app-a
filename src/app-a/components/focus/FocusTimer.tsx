...
export default function FocusTimer({ item, language, defaultMinutes, onClose }: { item: DailyPlanItem; language: AppALanguage; defaultMinutes: 15 | 25 | 45 | 60; onClose: () => void }) {
  const sanitizeMinutes = (val: number): number => {
    if (!Number.isFinite(val) || val <= 0) return 25;
    return Math.max(1, Math.min(180, Math.round(val)));
  };

  const safeDefault = sanitizeMinutes(defaultMinutes);
  const suggested = sanitizeMinutes(Math.max(5, Math.min(90, item.estimatedMinutes)));
  const [minutes, setMinutes] = useState<number>(safeDefault);
  const [remaining, setRemaining] = useState<number>(safeDefault * 60);

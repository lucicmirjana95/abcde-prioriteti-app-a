...
export default function FocusTimer({ item, language, onClose }: { item: DailyPlanItem; language: AppALanguage; onClose: () => void }) {
  const suggested = Math.max(5, Math.min(90, item.estimatedMinutes));
  const [minutes, setMinutes] = useState(suggested);
  const [remaining, setRemaining] = useState(suggested * 60);

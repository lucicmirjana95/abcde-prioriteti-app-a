...
export function validatePlanDraft(draft: DailyPlanDraft, knownQuestions?: ClarificationQuestion[]): ValidationResult {
  ...
  if (draft.visionSuggestion) {
    const vs = draft.visionSuggestion;
    if (vs.confidence !== "medium" && vs.confidence !== "high") {
      errors.push("Vision suggestion confidence must be 'medium' or 'high'.");
    }
    if (!vs.suggestedTitle || typeof vs.suggestedTitle !== "string" || vs.suggestedTitle.trim().length === 0) {
      errors.push("Vision suggestion title must be a non-empty string.");
    }
    if (!vs.desiredOutcome || typeof vs.desiredOutcome !== "string" || vs.desiredOutcome.trim().length === 0) {
      errors.push("Vision suggestion desired outcome must be a non-empty string.");
    }
    if (!Array.isArray(vs.sourceItemIds) || vs.sourceItemIds.length === 0) {
      errors.push("Vision suggestion must reference at least one source item ID.");
    } else {
      for (const sid of vs.sourceItemIds) {
        if (!classifiedIds.has(sid)) {
          errors.push(`Vision suggestion references unknown source item ID: ${sid}`);
        }
      }
    }
  }
